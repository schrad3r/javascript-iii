...
let postTaxPrices = prices.map((element, index, wholeArray) => {
  var salesTax = wholeArray[index] * .07;
  return wholeArray[index] + salesTax;
});
...
let totalPopulation = populations.reduce((runningTotal, curElement, curIndex, wholeArray) => {
  return runningTotal += wholeArray[curIndex];
})
...
let ordersTotal = Array.from((orders) => {
    var newArr = [];
  for (var index in orders) {
    orders.total = orders[index].price + orders[index].tax;
...
let bobsTotal = purchases.reduce((runningTotal, curElement, curIndex, obj) => {
  return runningTotal += obj[curIndex];
})
